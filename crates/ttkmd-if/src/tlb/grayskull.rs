...
use super::{Ordering, SpecificTlbInfo};
...
pub fn get_tlb(device: &PciDevice, tlb_index: u32) -> Result<Tlb, PciError> {
    ...
    let output = match tlb_index {
        ...
        _ => {
            return Err(PciError::TlbOutOfRange {
                id: tlb_index as usize,
            });
        }
    };
...
pub fn tlb_info(device: &PciDevice) -> DeviceTlbInfo {
    ...
    if device.bar0_wc_size > 0 {
        tlb_config[0].memory_type = MemoryType::Wc;
    }
