    ...
    fn update_init_state(
        ...
        if !status.arc_status.is_waiting() {
            ...
            if !status.arc_status.has_error() {
                // GDDR starts powered off at boot; DRAM training only happens after GDDR
                // power-on, which occurs after Luwen's init sequence. Skip the telemetry
                // check and mark DRAM done immediately.
                for dram_status in status.dram_status.wait_status.iter_mut() {
                    if let WaitStatus::Waiting(_) = dram_status {
