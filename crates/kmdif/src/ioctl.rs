const TENSTORRENT_DRIVER_VERSION: usize = 1;
...
nix::ioctl_readwrite_bad!(get_device_info, request_code_none!(TENSTORRENT_IOCTL_MAGIC, 0), GetDeviceInfo);
...
impl<const N: usize> Default for QueryMappingsOut<N> {
    fn default() -> Self {
        Self { mappings: [Mapping::default(); N] }
    }
}
...
pub unsafe fn query_mappings<const N: usize>(fd: nix::libc::c_int, data: *mut QueryMappings<N>) -> nix::Result<nix::libc::c_int> {
	nix::convert_ioctl_res!(nix::libc::ioctl(fd, request_code_none!(TENSTORRENT_IOCTL_MAGIC, 2) as nix::sys::ioctl::ioctl_num_type, data))
}
...
nix::ioctl_readwrite_bad!(allocate_dma_buffer, request_code_none!(TENSTORRENT_IOCTL_MAGIC, 3), AllocateDmaBuffer);
...
nix::ioctl_readwrite_bad!(free_dma_buffer, request_code_none!(TENSTORRENT_IOCTL_MAGIC, 4), FreeDmaBuffer);
...
nix::ioctl_readwrite_bad!(get_driver_info, request_code_none!(TENSTORRENT_IOCTL_MAGIC, 5), GetDriverInfo);
