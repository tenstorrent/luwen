...
use luwen_pcie::{DmaConfig, ExtendedPciDeviceWrapper};
...
#[pyclass]
pub struct DmaBuffer(luwen_pcie::DmaBuffer);
...
impl PciChip {
    ...
    pub fn new(pci_interface: Option<usize>) -> PyResult<Self> {
        ...
        let chip = luwen_pcie::ExtendedPciDevice::open(pci_interface).map_err(|v| {
        ...
        Ok(PciChip(
            luwen_api::chip::Chip::open(
                ...
                luwen_api::CallbackStorage {
                    callback: luwen_pcie::comms_callback,
                    user_data: chip,
                },
            )
...
pub fn detect_chips_fallible(
    ...
    let all_devices = luwen_pcie::PciDevice::scan();
