...
use luwen::api::{chip::HlComms, CallbackStorage, ChipImpl};
use luwen::pci::error::LuwenError;
use rand::Rng;
...
fn read_write_test(
    ...
    if use_dma {
        let pci = chip
            ...
            .1
            .as_any()
            .downcast_ref::<CallbackStorage<luwen::pci::ExtendedPciDeviceWrapper>>()
            .unwrap();

        let pci_interface: &mut luwen::pci::ExtendedPciDevice = &mut pci.user_data.borrow_mut();

        let dma_request = luwen::api::chip::HlCommsInterface::axi_translate(
            &chip,
            "ARC_CSM.ARC_PCIE_DMA_REQUEST",
        )?;
        let arc_misc_cntl =
            luwen::api::chip::HlCommsInterface::axi_translate(&chip, "ARC_RESET.ARC_MISC_CNTL")?;

        pci_interface.device.dma_config = Some(luwen::pci::DmaConfig {
...
pub fn main() -> Result<(), LuwenError> {
    let chips = luwen::pci::detect_chips()?;
