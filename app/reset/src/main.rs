...
fn main() {
    ...
    if !new_interfaces.is_empty() || !not_found_interfaces.is_empty() {
        if !new_interfaces.is_empty() && !not_found_interfaces.is_empty() {
            panic!("We have had interfaces both appearing and dissapearing (interfaces that vanished: {new_interfaces:?}; interfaces that vanished: {not_found_interfaces:?})");
        } else if !new_interfaces.is_empty() {
            panic!(
                "Some new interfaces appeared?!? (interfaces that vanished: {new_interfaces:?})"
            );
